...
export const DateTimePicker: React.SFC<IDateTimePickerProps> = (props) => {
  ...
  const years = useMemo(
    () => toDataArray(getYears({value: innerValue, minTime, maxTime})),
    [innerValue, minTime, maxTime],
  );
  const months = useMemo(
    () =>
      toDataArray(getMonths({value: innerValue, minTime, maxTime}), false, 1),
    [innerValue, minTime, maxTime],
  );
  const dates = useMemo(
    () => toDataArray(getDates({value: innerValue, minTime, maxTime})),
    [innerValue, minTime, maxTime],
  );
  const hours = useMemo(
  ...
  return (
    <Picker
      ...
      onChange={(
        ...
        setInnerValue(
          dayjs(new Date(year, month, date, hour, minute, 0, 0)).unix(),
        );
      }}
      onConfirm={(newValue) => {
        if (onChange) {
...
function getHours(options: IOptions) {
  ...
  const min =
    minTime && value.isSame(minTime, "date") ? minTime.hour() : undefined;
  const max =
    maxTime && value.isSame(maxTime, "date") ? maxTime.hour() : undefined;
...
function getMinutes(options: IOptions) {
  const {value, minTime, maxTime} = parseOptions(options);
  const min =
    minTime && value.isSame(minTime, "hour") ? minTime.minute() : undefined;
  const max =
    maxTime && value.isSame(maxTime, "hour") ? maxTime.minute() : undefined;
...
function getDates(options: IOptions) {
  ...
  const min =
    minTime && value.isSame(minTime, "month") ? minTime.date() : undefined;
  const max =
    maxTime && value.isSame(maxTime, "month") ? maxTime.date() : undefined;
...
function getMonths(options: IOptions) {
  const {value, minTime, maxTime} = parseOptions(options);
  const min =
    minTime && value.isSame(minTime, "year") ? minTime.month() : undefined;
  const max =
    maxTime && value.isSame(maxTime, "year") ? maxTime.month() : undefined;
